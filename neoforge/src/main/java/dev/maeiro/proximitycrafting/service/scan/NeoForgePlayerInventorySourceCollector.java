...
public final class NeoForgePlayerInventorySourceCollector implements PlayerInventorySourceCollector {
	@Override
	public List<ItemSourceRef> collectPlayerInventorySources(Player player, ScanOptions scanOptions) {
		...
		for (int slot = 0; slot < playerSlots; slot++) {
			sources.add(new ItemSourceRef(new NeoForgeItemSourceSlot(inventory, slot), ItemSourceRef.SourceType.PLAYER, null));
		}
		return sources;
	}
}
