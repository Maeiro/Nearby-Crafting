...
public class C2SAdjustRecipeLoad {
	...
	public void handle(Supplier<NetworkManager.PacketContext> ctxSupplier) {
		...
		ctx.queue(() -> {
			...
			boolean shouldSendSnapshot = result.success() && result.craftedAmount() > 0 && menu.shouldSendSnapshotForAdjust();
			...
			if (shouldSendSnapshot) {
				...
				var snapshotEntries = menu.getServerRecipeBookSnapshot(false, "packet_adjust_result");
